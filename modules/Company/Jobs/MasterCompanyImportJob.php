...
class MasterCompanyImportJob implements ShouldQueue
{
     ...
     * Execute the job.
     */
    public function handle(MasterCompanyAction $action): void
    {
        ...
        try {
            ...
            if (isset($payload[0]['name']) && isset($payload[0]['value'])) {
                $items = $payload;
            } elseif (isset($payload['region']) || isset($payload['branch']) || isset($payload['login'])) {
                $items = [['name' => 'COMPANIES', 'value' => $payload]];
            } else {
            ...
            Log::error("Fallo crítico en el Job de Importación de Empresas: " . $e->getMessage());
