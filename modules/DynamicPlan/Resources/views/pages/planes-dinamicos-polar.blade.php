<x-filament-panels::page>
        ...
        <form wire:submit="submitImport" class="flex items-end gap-4">
            <div class="flex-1">
                <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Archivo Excel / CSV de Planes Dinámicos Polar
                </label>
                <input 
                    type="file" 
    ...
    {{-- Tabs de datos y Botón de Sincronización --}}
    <div x-data="{ tab: 'data' }">
        <div class="flex items-center justify-between gap-4">
            <x-filament::tabs class="flex-1">
                ...
                <x-filament::tabs.item @click="tab = 'import-report'" alpine-active="tab === 'import-report'">
                    REPORTE CARGAS PLANES
                </x-filament::tabs.item>
            </x-filament::tabs>

            {{-- Botón de Sincronización Estilo Master Clientes --}}
            <button 
                type="button"
                wire:click="syncToTenants"
                wire:loading.attr="disabled"
                wire:target="syncToTenants"
                wire:confirm="¿Estás seguro de que deseas sincronizar los planes dinámicos a todos los tenants?"
            ...
            >
                ...
                <span wire:loading.remove wire:target="syncToTenants">SINCRONIZAR PLANES DINÁMICOS</span>
        ...
        <div class="mt-6">
            <div x-show="tab === 'data'">
                @livewire('dynamicplan::planes-dinamicos-table')
            </div>
            
            <div x-show="tab === 'import-report'" x-cloak>
                @livewire(\App\Livewire\BulkImportHistory::class, ['type' => 'planes_dinamicos'])
            </div>
        </div>
    </div>
</x-filament-panels::page>
